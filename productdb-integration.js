(function () {
  ...
  const stats = products.reduce((acc, product) => {
    ...
    sourceTotal: meta.sourceTotal || rows.length,
    bundledCount: rows.length,
    outputCount: 0,
    rejectedCount: rows.length - products.length,
    flagCounts: {},
    categoryCoverage: {}
  });
  ...
  function productImage(product) {
    ...
    return `
      <div class="product-card__image category-template-product-card__image product-card__image--photo">
        <img src="${escapeHtml(product.image.src)}" alt="${escapeHtml(product.image.alt)}" loading="lazy" decoding="async" />
      </div>
    `;
  }
  ...
  function renderCategory() {
    ...
    if (head) {
      ...
      if (title) title.textContent = "Sản phẩm ghế văn phòng từ ProductDB";
      if (desc) desc.textContent = "Danh sách đang dùng dữ liệu thật đã normalize qua Product Card contract. Bundle này là bản static chỉ đọc, chưa ghi hoặc sửa ProductDB.";
    }

    const heroStats = page.querySelector(".category-template-stats");
    if (heroStats) {
      heroStats.innerHTML = `
        <span><strong>${categoryProducts.length}</strong> sản phẩm ghế</span>
        <span><strong>${rows.length}</strong> dòng bundle</span>
        <span><strong>${stats.sourceTotal}</strong> dòng nguồn</span>
      `;
    }
    ...
    status.innerHTML = `
      <span><strong>${categoryProducts.length}</strong> card ghế văn phòng đang render từ ProductDB bundle.</span>
      <span>Nguồn chỉ đọc: ${escapeHtml(meta.source || "ProductDB export")}</span>
    `;
  ...
  function findProduct() {
    ...
    return products.find((product) => product.code.toLowerCase() === code.toLowerCase()) ||
      products.find((product) => adapter.slugify(`${product.code}-${product.name}`) === slug) ||
      products[0];
  }
  ...
  function renderDetail() {
    ...
    const categoryLink = page.querySelector(".product-detail-breadcrumb a[href='category.html']");
    if (categoryLink) categoryLink.textContent = product.category;

    const metaBox = page.querySelector(".product-summary__meta");
    if (metaBox) {
      metaBox.innerHTML = `
        <span>Mã: ${escapeHtml(product.code)}</span>
        <span>Danh mục: ${escapeHtml(product.category)}</span>
        <span>Nhóm: ${escapeHtml(product.subCategory || "Đang cập nhật")}</span>
      `;
    }
    ...
    if (table) {
      table.innerHTML = `
        <tr><th scope="row">Mã sản phẩm</th><td>${escapeHtml(product.code)}</td></tr>
        <tr><th scope="row">Tên sản phẩm</th><td>${escapeHtml(product.name)}</td></tr>
        <tr><th scope="row">Danh mục</th><td>${escapeHtml(product.category)}${product.subCategory ? ` / ${escapeHtml(product.subCategory)}` : ""}</td></tr>
        <tr><th scope="row">Kích thước</th><td>${escapeHtml(product.meta?.size || "Đang cập nhật")}</td></tr>
        <tr><th scope="row">Chất liệu</th><td>${escapeHtml(product.meta?.material || "Đang cập nhật")}</td></tr>
        <tr><th scope="row">Tình trạng giá</th><td>${escapeHtml(product.price?.label || "Liên hệ báo giá")}</td></tr>
      `;
    }
